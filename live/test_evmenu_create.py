...
"""Test EvMenu-based character creation"""
import socket
import time
...
def main():
    ...
    # Directly enter character name
    print("\n=== Entering character name 'Harath' ===")
    response = send_command(sock, "Harath", wait=0.5)
    print(f"Response contains 'Step 2': {'Step 2' in response or 'Race' in response}")
    print(f"Response preview:\n{response[:800]}")
    
    # Select a race - Saiyan (option 1)
    print("\n=== Selecting race '1' for Saiyan ===")
    response = send_command(sock, "1", wait=0.5)
    print(f"Response contains 'Step 3': {'Step 3' in response or 'Hair' in response}")
    print(f"Response preview:\n{response[:800]}")
