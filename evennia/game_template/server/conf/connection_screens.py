...
def _fallback_logo_ascii():
    """
    Handcrafted colored ASCII fallback shown when no image asset is present.
    """
    return "\n".join(
        [
            "                |y██████╗ ██████╗  █████╗  ██████╗  ██████╗ ███╗   ██╗|n",
...
def connection_screen():
    ...
    reset = "\x1b[0m"
    dragonball_word = f"{yellow}Drag{orange}({red}*{orange}){red}nball{reset}"
    forged_word = f"{yellow}Forg{red}ed{reset}"
    tagline = (
        " |wWelcome to |n"
        f"{dragonball_word} "
        f"{forged_word} "
